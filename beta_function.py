...
# Yukawa couplings matrix for down up quarks and charged leptons in SM 
...
#################################################################################################
    ...
    for name in matrix_names:
        ...
        for i, j, k, l in indices:
            ...
            if value != 0:
                for perm in permutations([j, k, l]):  
                    sign = (-1) ** len([p for p in zip(perm, [j, k, l]) if p[0] != p[1]])  # Sign from swaps
...
#################################################################################################
    ...
    Tprime=(3*np.trace(Yd @ Yd.conj().T @ Yd @ Yd.conj().T) 
...
# beta function for SM parameters  we can see the Appendix of 2411.08011
#################################################################################################
    ...
    Beta["Lambda"] = 3/8*gp**4 + 9/8*g**4+ 3/4*g**2*gp**2- 2*Tprime + (-3*gp**2-9*g**2+24*Lambda+4*WH)*Lambda
...
#################################################################################################
    ...
    ##########################################################################################################################
     ...
     RGEs of dim-7 SMEFT operators are computed in 1607.07309, 1901.10302 and 2310.11055 
     RGEs of dim-5 SMEFT operators can be found in hep-ph/0108005 and 2306.03008
...
##########################################################################################################################################
    ...
    """Dimension-7 Weinberg-like operator"""
    for p, r in indices:
        Beta[f"LH_{p}{r}"] = (
...
####################################################################################################
    ...
    for p, r, s, t in indices:
        Beta[f"eQddH_{p}{r}{s}{t}"] = (  
            ...
            +1/2*np.tensordot(C_LdQQH[:, t-1, r-1, :]-2*C_LdQQH[:, s-1, :, r-1], Yl.conj().T[p-1, :],  axes=([0], [0])) @ Yd[:, s-1]          
...
####################################################################################################
    ...
    for p, r, s, t in indices:
        Beta[f"eLLLHA_{p}{r}{s}{t}"] = (
            ...
            -np.tensordot(Yu.conj().T[:,:], Yl.conj().T[p-1,s-1]*C_QuLLH[:,:,t-1,r-1],axes=([0, 1], [0, 1]))
            -np.tensordot(Yu.conj().T[:,:], Yl.conj().T[p-1,t-1]*C_QuLLH[:,:,r-1,s-1],axes=([0, 1], [0, 1]))
            +np.tensordot(Yu.conj().T[:,:], Yl.conj().T[p-1,t-1]*C_QuLLH[:,:,s-1,r-1],axes=([0, 1], [0, 1]))      
        )
####################################################################################################
